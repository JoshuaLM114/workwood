...
// repoRow is one base repo plus its on-disk clone state.
type repoRow struct {
	name   string
	branch string
	url    string
...
func (r *reposModel) rebuild() {
	...
	for _, repo := range r.m.pd.Repos {
		r.rows = append(r.rows, repoRow{
			...
			url:    repo.URL,
			state:  repos.ClassifyClone(r.m.cfg.BaseRepo(repo.Name)),
		})
	}
...
func (r *reposModel) View() string {
	if r.form != nil {
		return docStyle.Render(titleStyle.Render(i18n.T("tui.repos.add_title")) + "\n\n" + r.form.View() + "\n" + helpStyle.Render(i18n.T("tui.form_help")))
	}
	...
	if len(r.rows) == 0 {
		...
		header := fmt.Sprintf("   %-20s %-14s %-26s %s",
			i18n.T("tui.col.repo"), i18n.T("tui.col.branch"), i18n.T("tui.repos.col_url"), i18n.T("tui.repos.col_status"))
		...
		for i, row := range r.rows {
			...
			line := fmt.Sprintf("%s %-20s %-14s %-26s %s",
				marker, pad(row.name, 20), pad(orDash(row.branch), 14), pad(orDash(row.url), 26), stateLabel(row.state))
			switch {
			case i == r.cursor:
